...
void bookmark_curr_page_cb(void *data)
{
	...
	bookmarks *bkmks = (bookmarks*)shmat(shared_bookmarks, NULL, 0);
	int count;
	if (bkmks == (void*)-1) 
		perror("Error attaching to bookmarks shared memory");
	else if ( (count = bkmks[0].bookmarks_count) >= MAX_BOOKMARKS ) 
		fprintf(stderr, "Error: Bookmarks table is full\n");
	else {
		strcpy(bkmks[count].uri, curr_webpage);
		bkmks[0].bookmarks_count++;
	}
	if (shmdt(bkmks) == -1) {
		perror("Error detaching from bookmarks shared memory.");
	}
	return;
}
...
int run_tab(comm_channel channel, int index) {
	...
	browser_window *b_window = NULL;
	create_browser(URL_RENDERING_TAB, index, (G_CALLBACK) (&new_tab_created_cb),
			(G_CALLBACK) (&uri_entered_cb), &b_window, channel);
	...
	while (1) {
		...
		if (read_ret == -1 && errno != EAGAIN) {
			...
			case NEW_URI_ENTERED:
				uri = incoming_req.req.uri_req.uri;
				/* Do a bit of URL verification to allow URLs 
				 * that don't start with http://. 
				 * check for http and not http:// to allow for https://
				 */
				if (strncmp(uri, "http", 4) != 0) {
					// append http:// to the front
					char newuri[512];
					strcpy(newuri, "http://");
					strcat(newuri, uri);
					uri = newuri;
			...
			case TAB_KILLED:
				...
				break;
			default:
				printf("Error: invalid message received from router. type: %d; 						read_ret: %d", incoming_req.type, read_ret);
				break;
			}
		} else if (read_ret == 0) {
			// Router is dead, kill self.
			fprintf(stderr, "Router died without sending kill message. Ending processes");
			process_all_gtk_events();
			exit(1);
		}
		// Now handle GTK events
		process_single_gtk_event();
		// And wait 1ms so we don't hog the processor
		usleep(1000);
	}
}
...
int main() {
	...
	// Set up bookmarks shared memory segment
	const int bookmarks_key = 74;
	if ((shared_bookmarks = shmget(bookmarks_key,
			    MAX_BOOKMARKS * sizeof(bookmarks),
			    IPC_CREAT | 0777)) == -1) {
	...
	if (controller_pid == 0) {
		...
		// No controller - not much point of continuing.
		exit(1);
	}
	...
	if (flags == -1) perror("Error getting pipe flags");
	flags |= O_NONBLOCK;
	...
	while (1) {
		...
		if (read_ret == -1 && errno != EAGAIN) {
			...
			case CREATE_TAB:
				...
				if (tab_id == -1) {
				    fprintf(stderr, "Error: the maximum number of tabs allowed already exist.\n");
				    break;
			    	}
			...
			case NEW_URI_ENTERED:
				...
				if (destination > UNRECLAIMED_TAB_COUNTER || available_IDs[destination - 1] == 1) {
					fprintf(stderr, "Error: tried to change URL of non-existent tab\n");
				} else {
			...
			case TAB_KILLED:
				...
				*/
				for (i = 1; i <= UNRECLAIMED_TAB_COUNTER; i++) {
				    	if (available_IDs[i - 1] == 0 && write(router_to_tab[i - 1][1], &kill_req,
						sizeof(child_req_to_parent)) == -1)
		...
		do { // possibly more than one message in pipe, loop until it's empty.
			...
			if (read_ret == -1 && errno != EAGAIN) {
				...
				if ( write(router_to_tab[closed_index - 1][1], &incoming_req, sizeof(child_req_to_parent)) == -1 )
					perror("Error sending kill request to tab");
				else wait(NULL); // wait so no zombie processes exist.
